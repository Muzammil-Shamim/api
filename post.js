      ...
      if (postResponse.ok) {
        ...
        const tableRows = userPosts.map((post) => {
          return `<tr>
                        ...
                        <td>${post.body}</td>
                        <td>
                            <button type="button" class="btn btn-primary" onclick="seeComment(${post.id})">See Comment</button>
                        </td>
                    </tr>`;
        });
        ...
        console.error(
          "Failed to fetch user posts. Status: " + postResponse.status
        );
      }
    } catch (error) {
      console.error(
        "An error occurred while fetching or displaying user posts:",
        error
      );
    } finally {
